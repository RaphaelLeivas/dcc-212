...
def sum_of_squared_errors(alpha, beta, x, y):
    ...
    for i in range(n):
        e_sq = error(alpha, beta, x[i], y[i]) ** 2
        total_error += e_sq
...
# seu codigo aqui
residuals = error(a1, b1, x, y)
residuals
plt.scatter(x, residuals, alpha=0.8, edgecolors='k', s=80)
plt.plot(x, np.repeat([0], len(x)), color='magenta')
...
def error_verification(alpha, beta, x, y):
  ...
  # (maiores, menores)
  residuals = error(alpha, beta, x, y)
  return np.count_nonzero(residuals > 0), np.count_nonzero(residuals < 0)
