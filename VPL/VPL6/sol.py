...
def prob(x, mu, sigma, n):
  ...
  # 1 - p(X <= x)
  return ss.distributions.norm.sf(z_obs(x=x, mu=mu, sigma=sigma, n=n))
...
def t_obs(data):
  ...
  return data.loc[data['floors'] == 1, 'price'].mean() - data.loc[data['floors'] > 1, 'price'].mean()
...
def shuffling(data):
  N = 5000
  filtro = data['floors'] == 1 # pega so as casas de 1 andar
  diffs = np.zeros(N)
...
def resposta():
  ...
  if c_inf <= 0 <= c_sup: # sem diferença significativa
    return False
  else:
    return True
